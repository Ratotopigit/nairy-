...
import { resolveSessionId } from "@/lib/chat-history";
...
export default function OfferIQPage() {
  ...
  async function buildOffer() {
    ...
    try {
      const user = auth.currentUser;
      if (!user) {
        throw new Error("You need to be signed in to build an offer.");
      }
      ...
      try {
        ...
        const remoteOffer = ((candidate as { data?: unknown })?.data ?? (candidate as { offer?: unknown })?.offer ?? candidate) as OfferBlueprint;
        if (!remoteOffer?.title || !Array.isArray(remoteOffer.scope) || remoteOffer.scope.length === 0) {
          throw new Error("The Webinar Offer workflow returned an incomplete offer.");
        }
        offer = remoteOffer;
      } catch (webhookError) {
        if (webhookError instanceof DOMException && webhookError.name === "AbortError") {
          throw new Error("The Webinar Offer workflow timed out. Please try again.");
        }
        throw webhookError;
      } finally {
      ...
      sessionStorage.setItem(HANDOFF.offerFocus, focus);

      if (user) {
        await saveWorkspaceMemory(user.uid, {
          onboarding_complete: true,
          audience_profile: (buyer ?? {}) as Record<string, unknown>,
          offer_profile: offer as unknown as Record<string, unknown>,
          source_session_id: activeSession,
        }).catch((err) => console.warn("Could not save to workspace memory:", err));
      }
    } catch (buildError) {
  ...
  if (!buyer) {
    return (
      <div className="mx-auto max-w-2xl py-16 text-center">
        ...
        <h1 className="mt-5 text-2xl font-semibold tracking-[-0.03em] sm:text-3xl">Build your buyer blueprint first</h1>
        ...
        <Link href="/provider/webinar-chat" className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background">Open Webinar Chat <ArrowRight className="size-4" /></Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[1060px]">
      ...
      <div className="grid gap-4 lg:grid-cols-[390px_minmax(0,1fr)] lg:gap-6">
                  ...
                  return (
                    ...
                    >
                      <span className={`grid size-7 place-items-center rounded-lg text-xs ${active ? "bg-foreground text-background" : "bg-secondary"}`}>
                        {complete ? <Check className="size-3.5 text-primary" /> : index + 1}
                      </span>
                      <span className="text-xs">{label}</span>
                      {active && (
                        <span className="ml-auto font-mono text-[10px] uppercase tracking-[0.18em] text-ochre animate-pulse">
                          In progress
                        </span>
                      )}
                    </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><p className="mb-2 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{label}</p>{children}</div>;
}
