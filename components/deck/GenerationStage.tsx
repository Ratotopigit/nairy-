      ...
      <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
            ...
            {GENERATION_STEPS.map((label, index) => {
              ...
              return (
                ...
                >
                  ...
                  {active ? <span className="generation-working ml-auto font-mono text-[10px] uppercase tracking-[0.18em]">In progress</span> : null}
                </div>
              );
            })}
          </div>
        ...
        <div className="generation-stack relative mx-auto h-[260px] w-full max-w-[320px]" aria-hidden="true">
          {[0, 1, 2].map((index) => (
            <div
              key={index}
              className="generation-sheet absolute inset-x-0 mx-auto h-[190px] w-[280px] overflow-hidden rounded-2xl border border-border bg-card shadow-[0_18px_45px_rgba(29,42,39,0.12)]"
              style={{
                top: `${index * 24}px`,
            ...
            >
              ...
              <div className="grid h-full grid-cols-[1.1fr_0.9fr] gap-4 p-5">
                <div className="space-y-3">
                  ...
                  <div className="h-5 w-full rounded-md bg-foreground/85" />
                  <div className="h-5 w-4/5 rounded-md bg-foreground/85" />
                  <div className="mt-5 h-2 w-full rounded-full bg-border" />
                  <div className="h-2 w-3/4 rounded-full bg-border" />
                </div>
                <div className="rounded-xl bg-secondary p-3">
                  <div className="h-full rounded-lg border border-primary/15 bg-background" />
                </div>
              </div>
            </div>
          ))}
